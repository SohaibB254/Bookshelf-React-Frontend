  ...
  return (
    ...
    <div className=' font-inter w-full h-fit pt-12 flex justify-center dark:text-gray-300  ' >
        ...
        <form className=' flex flex-col gap-2'  >
            <input className="px-4 rounded border block dark:bg-gray-800 outline-none  dark:border-gray-800 py-2" type="email" name="email" placeholder="Email"  required/>
            <input className="px-4 rounded border block dark:bg-gray-800 outline-none  dark:border-gray-800 py-2" type={`${isShown?'text':'password'}`}  name="name" placeholder="Password" required/>
            <span onClick={handlePasswordType} className='inline-block cursor-pointer text-xs text-blue-500'>{isShown?'Hide Password':'Show Password'}</span>

            <input className={`px-4 rounded border block  dark:border-gray-800 py-2
