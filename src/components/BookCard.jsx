      ...
      <div className='border font-sans flex flex-col sm:flex-row
       w-[100%] sm:gap-20 mt-10 text-[24px]  sm:p-12 p-8'>
        ...
        <div className='text-[0.8em] w-full '>
          <div className=''>
            <h1 className='sm:text-[2.25em] text-[1.5em] font-semibold'>{checkoutItem.title}</h1>

            <h1 className='font-bold text-[1.5em] flex items-center justify-between sm:w-72 '>
              <p className='text-green-500'>Rs: {checkoutItem.price - (checkoutItem.price * checkoutItem.sale_percent) / 100}
                <span className={`${checkoutItem.sale_percent === 0 ? 'hidden' : ''} text-zinc-500 italic line-through`}>{checkoutItem.price}</span>
              </p>
                <p className={`${checkoutItem.sale_percent === 0 ? 'hidden' : ''}`}><span className='text-red-500 sm:text-2xl text-base font-bold'>{checkoutItem.sale_percent}% OFF</span></p>
            </h1>
          </div>
