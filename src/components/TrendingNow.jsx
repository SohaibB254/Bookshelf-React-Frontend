...
const TrendingNow = () => {
  ...
  return (
    <div className="h-auto font-inter text-[30px]  px-8 sm:px-12 pt-10">
      <h1 className="font-semibold lg:text-[1.3em] text-2xl mb-10">Trending Now</h1>
          ...
          return (
            ...
            >
              ...
              <div className=" z-10 flex absolute items-center justify-center  w-full text-sm bg-black px-8 bottom-0 py-2  gap-4">
                ...
                <Link to={"/checkout"}>
                  <button className="underline hover:text-green-500  px-3 py-1 text-white">
                    Checkout
                  </button>
                </Link>
              </div>
              <div
                id="bookDetails"
                className="w-44 text-[18px] absolute right-0 top-0 flex flex-col justify-center  gap-4 px-3 translate-x-[100%] group-hover:translate-x-[0] transition duration-500 bg-white/40  backdrop-blur-md h-full"
              >
                <p>by: {elm.author}</p>
                <p>{elm.length}</p>
                <p className="text-green-500">Rs: {elm.price}</p>
              </div>
            </div>
          );
        })}
      ...
      <Link
        ...
        className="text-base text-center my-4 sm:mx-0 mx-8 hover:underline"
      >
        See More
      </Link>
    </div>
